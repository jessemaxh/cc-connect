...
func (e *Engine) handleWorkspaceCommand(p Platform, msg *Message, args []string) {
	...
	case "bind":
		...
		if strings.Contains(wsName, "..") || strings.ContainsAny(wsName, "/\\") {
			e.reply(p, msg.ReplyCtx, "Invalid workspace name.")
			return
		}
		wsPath := filepath.Join(e.baseDir, wsName)
		absPath, _ := filepath.Abs(wsPath)
		absBase, _ := filepath.Abs(e.baseDir)
	...
	case "init":
		...
		absClone, _ := filepath.Abs(cloneTo)
		absBase, _ := filepath.Abs(e.baseDir)
		if !strings.HasPrefix(absClone, absBase+string(filepath.Separator)) {
			e.reply(p, msg.ReplyCtx, "Invalid repository name.")
			return
		}
